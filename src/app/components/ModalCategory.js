...
export default function ModalCategory({isModalCategoryOpen, closeCategory, tabCategory, showBooks, showChapters}){
    return(
        <div className={`modal-category-container text-dark4 dark:text-light1 w-[100svw] h-[100svh] z-[100] xl:hidden ${isModalCategoryOpen?"absolute top-0 left-0":"hidden"}`}>
            ...
            <div className={`modal-category bg-light1 dark:bg-dark3 w-[100svw] xl:w-[300px] h-[100svh] rounded-r-lg shadow-lg overflow-scroll z-[150] absolute top-0 ${isModalCategoryOpen? "left-0":"-left-full"}`}>
            ...
            <div className="tab-container flex">
                <button className={`book w-full p-3 transition ${tabCategory? "bg-accent2 text-light1":"bg-light1 dark:bg-dark3 dark:text-light1"}`} onClick={showBooks}>বই</button>
                <button className={`chapter w-full p-3 transition ${tabCategory? "bg-light1 dark:bg-dark3 dark:text-light1":"bg-accent2 text-light1"}`} onClick={showChapters}>অধ্যায়</button>
            </div>
            <Line/>
