...
export default function ModalSearch({isModalSearchOpen, closeSearch}){
    return(
        <div className={`modal-search-container text-dark4 dark:text-light1 w-[100svw] h-[100svh] z-[100] xl:hidden ${isModalSearchOpen?"absolute top-0 left-0":"hidden"}`}>
            ...
            <div className={`modal-search w-[80svw] xl:w-auto bg-light1 dark:bg-dark3 rounded-lg shadow-lg z-[150] p-5 absolute ${isModalSearchOpen? "top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2":""}`}>
                ...
                <div className="btn-container w-full flex gap-3">
                    <button className="btn-search-close bg-accent2 text-light1 bangla py-3 w-full rounded-lg" onClick={closeSearch}>বাতিল</button>
                    <button className="btn-search-search bg-light3 dark:bg-dark1 dark:text-light1 bangla py-3 w-full rounded-lg">সার্চ করুন</button>
                </div>
            </div>
        </div>
    )
