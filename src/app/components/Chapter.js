...
export default function Chapter({isActive, chapter_id, title, hadis_range}){

    return(
        <button className={`chapter text-left dark:text-light3 group p-3 mx-3 rounded-lg flex items-center gap-3 transition ${isActive?"bg-accent1 dark:bg-dark1":"bg-light1 dark:bg-dark3 hover:bg-accent1 dark:hover:bg-dark1"}`}>
            <div className="relative w-12 h-12">
                ...
                <div className={`text mt-[2px] font-bold absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 ${isActive? "text-light1":"text-light4 group-hover:text-light1"}`}>{chapter_id}</div>
            </div>
            <div className="text bangla">
                <div className="title text-lg dark:text-light3 font-medium">{title}</div>
                <div className="subtitle text-sm text-light4">হাদিসের রেঞ্জ {hadis_range}</div>
            </div>
        </button>
    )
}