...
export default function BtnPhoneNav({iconOutlined, iconFilled, isActive, isSupport}){
    return(
        <button className={`btn-nav dark:text-light1 group w-10 h-10 flex justify-center items-center rounded-lg transition ${isActive?"bg-accent2 text-light1":""} ${isSupport?"absolute bottom-6 left-1/2 -translate-x-1/2":""}`}>
